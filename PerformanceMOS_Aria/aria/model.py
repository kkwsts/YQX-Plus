...
class TransformerREG(nn.Module):
    """Transformer decoder with a regression head for score prediction.
    ...
    def __init__(self, model_config: ModelConfig):
        ...
        # Regression head: transformer output -> single score value
        self.regression_head = nn.Sequential(
            nn.Linear(model_config.d_model, model_config.d_model // 2),
            nn.ReLU(),
            nn.Linear(model_config.d_model // 2, 1),
            nn.Sigmoid(),  # Output range [0, 1]
        )

    def forward(self, src: torch.Tensor):
        """Compute regression output for score prediction following paper specifications.

        Args:
            src (torch.Tensor): Input tensor of token indices with shape (batch_size, seq_len).

        Returns:
            torch.Tensor: Predicted scores with shape (batch_size, 1), range [0, 1].
        """
        hidden = self.model(src)  # (batch_size, seq_len, d_model)

        # Use end-of-sequence token as specified in paper
        # Find the last non-padding token for each sequence
        batch_size, seq_len = src.shape

        # For each sequence, find the position of the EOS token or last token
        eos_positions = []
        for i in range(batch_size):
            # Find the last non-padding position (assuming pad token is 0)
            non_pad_mask = src[i] != 0
            if non_pad_mask.any():
                eos_pos = non_pad_mask.nonzero()[-1].item()
            else:
                eos_pos = seq_len - 1  # fallback to last position
            eos_positions.append(eos_pos)

        # Extract features from end-of-sequence positions
        batch_indices = torch.arange(batch_size, device=src.device)
        eos_positions = torch.tensor(eos_positions, device=src.device)
        eos_features = hidden[
            batch_indices, eos_positions
        ]  # (batch_size, d_model)

        # Regression prediction from EOS token features
        score = self.regression_head(eos_features)  # (batch_size, 1)
