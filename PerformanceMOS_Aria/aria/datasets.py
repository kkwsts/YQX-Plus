...
class TrainingDataset(torch.utils.data.Dataset):
    ...
    def get_loss_mask(self, src_seq: list, tgt_seq: list, offset: int = 0):
    ...
    def __getitem__(self, idx: int):
        ...
        entry_raw = mmap_obj.readline()
        entry_dict = json.loads(entry_raw)

        seq = entry_dict["seq"]  # Load raw seq
...
def _get_seqs(
    ...
    _file_path = _midi_dict.metadata["abs_load_path"]
    ...
    try:
        ...
        return _tokenized_seq, _file_path
...
def get_seqs(
    ...
    # Can't pickle generator object when start method is spawn
    if multiprocessing.get_start_method() == "spawn":
        logging.info(
...
# GOAL: Modify this and then rename it, removing ft-dataset from codebase
class PretrainingDataset(TrainingDataset):
    ...
    def build(
        ...
        file_embeddings: dict | None = None,
    ):
        """Builds and returns PretrainingDataset."""

        def _build_concat_epoch(_save_path: str, _midi_dataset: Iterable):
            # Sequences are concatenated and sliced
            with jsonlines.open(_save_path, mode="w") as writer:
                ...
                seq_buffer = []
                _idx = 0
                for entry, file_path in reservoir(
                    get_seqs(tokenizer, _midi_dataset), 10
                ):
                    seq_buffer += entry

                    while len(seq_buffer) >= max_seq_len:
                        writer.write({"seq": seq_buffer[:max_seq_len]})
                        seq_buffer = seq_buffer[max_seq_len:]
                ...
                if seq_buffer:
                    seq_buffer += [tokenizer.pad_tok] * (
                        max_seq_len - len(seq_buffer)
                    )
                    writer.write({"seq": seq_buffer[:max_seq_len]})

        def _build_epoch_separated(
            _save_path: str,
            _midi_dataset: Iterable,
            _file_embeddings: dict | None,
        ):
            # Sequences always start with a new entry (requires padding)
            with jsonlines.open(_save_path, mode="w") as writer:
                writer.write(
                    {
                        "tokenizer_config": tokenizer.config,
                ...
                for entry, file_path in reservoir(
                    ...
                    seq_buffer = entry
                    embedding_data = (
                        {"emb": _file_embeddings[file_path]}
                        if _file_embeddings
                        else {}
                    )
        ...
        for idx in range(num_epochs):
            ...
            if separate_sequences is True:
                _build_epoch_separated(
                    _save_path=os.path.join(save_dir, f"epoch{idx}.jsonl"),
                    _midi_dataset=midi_dataset,
                    _file_embeddings=file_embeddings,
                )
            else:
                _build_concat_epoch(
                    _save_path=os.path.join(save_dir, f"epoch{idx}.jsonl"),
...
class RegressionDataset(torch.utils.data.Dataset):
    """Dataset for score prediction regression tasks using YCU-PPE data."""
    ...
    def __init__(
        ...
        split: str = "train",  # "train", "test", or None for all
    ):
        ...
        from aria.utils import normalize_score
    ...
    def __getitem__(self, idx):
        ...
        # Get MIDI file path
        midi_filename = row["filename"] + ".mid"
        midi_path = os.path.join(self.midi_dir, midi_filename)

        # Load and tokenize MIDI
        try:
            midi_dict = MidiDict.from_midi(midi_path)
            tokenized_seq = self.tokenizer.tokenize(midi_dict)
        except Exception as e:
            self.logger.warning(f"Failed to process {midi_path}: {e}")
            # Return dummy data for failed files
            tokenized_seq = [self.tokenizer.bos_tok, self.tokenizer.eos_tok]

        # Truncate and pad sequence
        seq = tokenized_seq[: self.max_seq_len]
        if self.tokenizer.eos_tok not in seq:
            seq[-1] = self.tokenizer.eos_tok

        seq = seq + [self.tokenizer.pad_tok] * (self.max_seq_len - len(seq))

        # Get normalized score (average of 3 evaluators)
        score = row["score_average"]
        normalized_score = self.normalize_score(score)

        return (
            torch.tensor(self.tokenizer.encode(seq)),
            torch.tensor([normalized_score], dtype=torch.float32),
        )


class ASAPRegressionDataset(torch.utils.data.Dataset):
    """Dataset for score prediction regression tasks using ASAP data."""
    ...
    def __getitem__(self, idx):
        ...
        # Get MIDI file path - ASAP format is just filename.mid
        ...
        # Get normalized score (already assigned as 60 or 100)
        score = row["score"]
        normalized_score = self.normalize_score(score)
...
class CombinedRegressionDataset(torch.utils.data.Dataset):
    """Combined dataset for YCU-PPE and ASAP data."""
    ...
    def __init__(
        ...
        # Load YCU dataset
        self.ycu_dataset = RegressionDataset(
            midi_dir=ycu_midi_dir,
            csv_file=ycu_csv_file,
            tokenizer=tokenizer,
            max_seq_len=max_seq_len,
            split=split,
        )
        ...
        # Load ASAP dataset
        self.asap_dataset = ASAPRegressionDataset(
            midi_dir=asap_midi_dir,
            csv_file=asap_csv_file,
            tokenizer=tokenizer,
            max_seq_len=max_seq_len,
            split=split,
        )

        self.ycu_len = len(self.ycu_dataset)
        self.asap_len = len(self.asap_dataset)
        self.total_len = self.ycu_len + self.asap_len

        self.logger.info(
            f"Combined dataset - YCU: {self.ycu_len}, ASAP: {self.asap_len}, Total: {self.total_len}"
        )
    ...
    def __getitem__(self, idx):
        ...
        if idx < self.ycu_len:
            return self.ycu_dataset[idx]
        else:
            return self.asap_dataset[idx - self.ycu_len]
